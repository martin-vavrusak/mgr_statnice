...
var stickyHandler = function() {
	...
	/* Right sticky scyscraper */
	var adOffsetR = $(".ad-skyscraper-sticky-right").offset();
	if (adOffsetR) {
		$('.ad-skyscraper-sticky-right .skyscraper-sticky').css({
			'left': ($(this).scrollLeft()* (-1)) + adOffsetR.left
		});
	}
	
	/* Sticky leader board */
	/* Not in use
	var adOffsetLeaderboard = $("body.adp-sticky-ramecek.adp-sticky-ramecek-lead-yes .adLeaderboard").offset();
	if (adOffsetLeaderboard) {
		$('body.adp-sticky-ramecek-lead-yes .adLeaderboard .ad').css({
			'left': ($(this).scrollLeft()* (-1)) + adOffsetLeaderboard.left
		});
	}
	*/
}
window.onresize = window.onscroll = window.load = stickyHandler;
